...
{
    private const string NotNullMessage = "The function provided to Result.Try returned null, which violates the non-null constraint.";
    ...
    public class UnwrapFailedException : Exception
    {
        ...
        public UnwrapFailedException(string message, T? value) : base(message)
        {
            ValueOrError = value;
        }
        ...
        public UnwrapFailedException(string message, E? error) : base(message)
        {
            ValueOrError = error;
        }
    }
    ...
    private sealed class OkResult(T value) : ResultBase<T>(value)
    {
        ...
        public override E UnwrapErr() => throw new UnwrapFailedException($"Cannot unwrap Err from {nameof(OkResult)}", InternalValue);
        ...
        public override E ExpectErr(string message) => throw new UnwrapFailedException(message, InternalValue);
    ...
    private sealed class ErrResult(E error) : ResultBase<E>(error)
    {
        ...
        public override Result<T, O> MapErr<O>(Func<E, O> op) => Result<T, O>.Err(op(InternalValue));
        public override Result<U, E> AndThen<U>(Func<T, Result<U, E>> op) => Result<U, E>.Err(InternalValue);
        public override Result<T, E> OrElse(Func<E, Result<T, E>> op) => op(InternalValue);
        public override T Unwrap() => throw new UnwrapFailedException($"Cannot unwrap Ok from {nameof(ErrResult)}", InternalValue);
        ...
        public override T Expect(string message) => throw new UnwrapFailedException(message, InternalValue);

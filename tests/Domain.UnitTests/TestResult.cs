...
public class ResultTests
{
    ...
    private static Result<string, string> SquareThenToString(int x) => Result<int, string>.Try(() =>
        {
            checked { return x * x; }
        }).Map(sq => sq.ToString()).MapErr(_ => "Overflowed");
    ...
    [InlineData(2, true)]
    [InlineData(0, false)]
    public void Result_WhenOkAndCheckedAgainstPredicate_ShouldMatchPredicate(int value, bool expectedResult)
    {
    ...
    {
        Result<int, Error>.Err(GetTestError())
            .IsErrAnd(x => expectedResult ? x.GetType() == typeof(Error) : x.GetType() == typeof(Exception))
    ...
    {
        Result<int, Error>.Ok(-3).IsErrAnd(x => x.GetType() == typeof(Error)).Should().BeFalse();
    }
    ...
    {
        Result<int, Exception>.Try(() => double.Parse(input)).Map(i => i * 2).Should().Be(Result<double, Exception>.Ok(expected));
    }
    ...
    public void Result_WhenErrAndMapped_ShouldLeaveErrUntouched()
    {
        ...
        error.Should().BeOfType<FormatException>().Which.Message.Should().Be("The input string 'Nothing here' was not in a correct format.");
    }
    ...
    {
        Result<int, int>.Ok(value).MapErr(i => i.ToString()).Should().Be(Result<int, string>.Ok(value));
    }
    ...
    {
        Result<int, int>.Err(input).MapErr(i => i.ToString()).Should().Be(Result<int, string>.Err(expected));
    }
    ...
    public void Result_WhenErrAndExpected_ShouldThrowWithMessage()
    {
        ...
        act.Should().Throw<Result<int, string>.UnwrapFailedException>().WithMessage(expectedMessage);
    }
    ...
    public void Result_WhenErrAndUnwrapped_ShouldThrowUnwrapFailedException()
    {
        ...
        act.Should().Throw<Result<int, string>.UnwrapFailedException>()
    ...
    {
        Result<int, Exception>.Try(() => double.Parse(input)).UnwrapOrDefault().Should().Be(expected);
    }

    [Fact]
    public void Result_WhenErrAndUnwrappedOrDefault_ShouldReturnDefault()
    {
        Result<int, Exception>.Try(() => double.Parse("1900foo")).UnwrapOrDefault().Should().Be(default);
    ...
    {
        Result<int, string>.Ok(input).AndThen(SquareThenToString).Should().Be(Result<string, string>.Ok(expected));
    }

    [Fact]
    public void Result_WhenOkAndAndThenOverflows_ShouldReturnErr()
    {
        Result<int, string>.Ok(1_000_000).AndThen(SquareThenToString).Should().Be(Result<string, string>.Err("Overflowed"));
    }
    ...
    public void Result_WhenErrAndAndThen_ShouldReturnErr()
    {
        ...
        Result<int, string>.Err(errorMessage).AndThen(SquareThenToString).Should().Be(Result<string, string>.Err(errorMessage));
    }
    ...
    {
        Result<int, int>.Ok(value).OrElse(Square).OrElse(Square).Should().Be(Result<int, int>.Ok(value));
    }
    ...
    {
        Result<int, int>.Ok(value).OrElse(Err).OrElse(Square).Should().Be(Result<int, int>.Ok(value));
    }
    ...
    {
        Result<int, int>.Err(input).OrElse(Square).OrElse(Err).Should().Be(Result<int, int>.Ok(expected));
    }
    ...
    {
        Result<int, int>.Err(value).OrElse(Err).OrElse(Err).Should().Be(Result<int, int>.Err(value));
    }
    ...
    {
        var result = Result<int, Exception>.Try<int>(() => throw new InvalidOperationException("Test exception"));
    ...
    public void Result_Try_WhenFunctionReturnsNull_ShouldThrowArgumentNullException()
    {
        ...
        error.Should().BeOfType<ArgumentNullException>()
             .Which.Message.Should().Be("The function provided to Result.Try returned null, which violates the non-null constraint. (Parameter 'func')");
    }
